 ...
 * 'C[+_[_], +_]'.
 *
 * The plus sign '+' in '+O', '+F[_]' and 'C[+_[_]' only indicates the contravariance of those type parameters. This is
 * in accordance to (and limited/dictated by) the trait 'DAO[I, +O]'. The output type has to be contravariant, which is
 * a general rule of the type system (i.e. parameters are contravariant, return types are covariant). We don't need to
 * fully understand this, it only needs to be consistent with the interface declaration (= the type system tells how to
 * do it).
 *
