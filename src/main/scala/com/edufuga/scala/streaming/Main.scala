...
object Main extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    for {
      ...
      _ <- ProductFileStreamingWithIODAO(products).readAll.evalTap(IO.println).compile.drain

      _ <- IO.println(s"Processing stream of services '$services'.")
      _ <- ServiceFileStreamingWithIODAO(services).readAll.evalTap(IO.println).compile.drain
      ...
      bingoProduct <- ProductFileStreamingWithIODAO(products).readById(ProductId("X716-6172862")).compile.last
      _ <- IO.println("Bingo product: " + bingoProduct)

      _ <- IO.println(s"Finding a service by ID within the stream of services '$services'.")
      bingoService <- ServiceFileStreamingWithIODAO(services).readById(ServiceId("Y274-1029755")).compile.last
